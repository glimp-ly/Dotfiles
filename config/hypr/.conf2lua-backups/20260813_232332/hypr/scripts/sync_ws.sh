...
hyprctl dispatch focusmonitor "$MON_SEC" > /dev/null
hyprctl dispatch workspace "$WS_SEC" > /dev/null

# 2. Enfoque explícito en el monitor principal y cambio de workspace (mantiene el foco aquí)
hyprctl dispatch focusmonitor "$MON_MAIN" > /dev/null
